...
const Index: React.FC = () => {
  ...
  if (!user) {
    return <View style={tw`flex-1 items-center justify-center bg-background`}><Text style={tw`text-foreground`}>Loading...</Text></View>;
  }
  ...
  return (
    <FlatList
      data={[]}
      renderItem={null}
      style={tw`flex-1 bg-background p-4`}
      ListHeaderComponent={
        <>
          {trialDaysRemaining > 0 ? (
            <View style={tw`bg-primary p-4 rounded-lg mb-4`}>
          ...
          {trialDaysRemaining === 0 && user.subscription_plan === 'trial' ? (
            <View style={tw`bg-secondary p-4 rounded-lg mb-4`}>
          ...
          {!user.phone_number ? (
            <View style={tw`bg-card border border-accent rounded-lg p-4`}>
              <Text style={tw`text-lg font-bold text-foreground mb-2`}>No Ghost Number Assigned</Text>
              <Text style={tw`text-foreground`}>
                It looks like you don&apos;t have a Ghost Number yet. Please visit the{" "}
                <Text style={tw`font-bold text-primary`} onPress={() => router.push({ pathname: '/settings' })}>Settings</Text> page to set one up.
              </Text>
              <TouchableOpacity
                onPress={handlePurchaseNumber}
                style={tw`mt-4 bg-primary p-3 rounded-lg items-center`}
            ...
            <View style={tw`grid gap-4`}>
              <View style={tw`bg-card border border-accent rounded-lg p-4`}>
                <Text style={tw`text-lg font-bold text-foreground`}>Your Ghost Number</Text>
                <Text style={tw`text-2xl text-accent font-mono mt-2`}><Text>{user.phone_number}</Text></Text>
              </View>
              <View style={tw`bg-card border border-accent rounded-lg p-4`}>
                <Text style={tw`text-lg font-bold text-foreground`}>Message Usage</Text>
                <Text style={tw`text-2xl text-accent font-mono mt-2`}>
                  <Text>{user.message_count}</Text> / {getMessageLimit(user.subscription_plan)}
                </Text>
              </View>
              {currentPlan && (
                <View style={tw`bg-card border border-accent rounded-lg p-4`}>
                  <Text style={tw`text-lg font-bold text-foreground`}>Subscription</Text>
                  <Text style={tw`text-2xl text-accent font-mono mt-2`}>
